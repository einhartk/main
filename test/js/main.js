...
function clearAllRaidsCompletely() {
  if (confirm('모든 공대 데이터를 초기화하시겠습니까?')) {
    state.raidTabs = {};
...
async function confirmShareRaid() {
  ...
  if (shareId.length < 3) {
    alert('공유 ID는 최소 3자 이상이어야 합니다.');
    return;
  }
  
  // 프로그레스 표시
  const progressModal = new bootstrap.Modal(document.getElementById('progressModal'));
  document.getElementById('progressTitle').textContent = '공유 중...';
  document.getElementById('progressMessage').textContent = '공대 데이터를 공유하고 있습니다.';
  progressModal.show();
  ...
  try {
    ...
    alert(`공대 데이터가 "${shareId}" ID로 공유되었습니다!`);
...
async function confirmDownloadRaid() {
  ...
  // 프로그레스 표시
  const progressModal = new bootstrap.Modal(document.getElementById('progressModal'));
  document.getElementById('progressTitle').textContent = '내려받기 중...';
  document.getElementById('progressMessage').textContent = '공대 데이터를 내려받고 있습니다.';
  progressModal.show();
  ...
  try {
    ...
    if (!doc.exists) {
      progressModal.hide();
      alert('해당 ID의 공유 데이터를 찾을 수 없습니다.');
      return;
    }
    ...
    // 프로그레스 메시지 업데이트
    document.getElementById('progressMessage').textContent = '데이터를 적용하고 있습니다...';
    
    // 데이터 적용 전 확인
    const confirmMessage = `"${downloadId}" ID의 공대 데이터를 내려받겠습니까?\n` +
    ...
    alert(`"${downloadId}" ID의 공대 데이터를 성공적으로 내려받았습니다!`);
    ...
    slot.ondrop = e => {
      ...
      try {
        ...
        if (characterInCurrentRaid) {
          ...
          alert(`이 캐릭터는 이미 현재 레이드(${state.selectedRaid?.name})에 배치되어 있습니다.\n(1레이드당 1캐릭터만 가능합니다)`);
          return;
...
async function fetchExpedition(index){
  ...
  try {
    ...
    alert(`원정대 조회 중 오류가 발생했습니다: ${error.message}`);
